"""Default MVP universe: the current S&P 500 constituents.

Per docs/PRD.md ("Universe: configurable U.S. large-cap list") and
CLAUDE.md rule 7 (the owner must not need to edit configuration), this
ships with a sensible default so the system runs out of the box. A
developer (not the non-technical owner) can point load_universe() at a
different file to change coverage without touching engine code.

Source: the "datasets/s-and-p-500-companies" community-maintained GitHub
dataset (mirrors Wikipedia's "List of S&P 500 companies", which this
environment could not fetch directly). Spot-checked against known tickers;
one transcription error was found and corrected (Marsh & McLennan listed
as "MRSH" -- corrected to "MMC", its actual ticker). A few entries reflect
genuine 2025-2026 corporate actions (e.g. FDXF for the FedEx Freight
spinoff, HONA for Honeywell Aerospace post-split, Q for DuPont's Qnity
Electronics spinoff) that may look unfamiliar but are current. Dual-class
tickers use yfinance's hyphen convention (BRK-B, not BRK.B).

Not exhaustively re-verified beyond that spot check -- provider fetches
already skip any ticker they can't find data for (see data_adapters.py)
rather than failing the whole run, so a handful of remaining errors here
would degrade coverage slightly, not break anything.
...
DEFAULT_UNIVERSE = [
    ...
    "LYB", "MTB", "MPC", "MAR", "MMC", "MLM", "MRVL", "MAS",
